...
public:
	...
	List_vector<T>& operator[](int i);
	const List_vector<T>& operator[]( int i) const;
...
template< class T >
int List_vector<T>::Length() const
{
	return length_;
}
...
template< class T >
List_vector<T>& List_vector<T>::operator[](int i)
{
	return elements_[i-1];
}

template< class T >
const List_vector<T>& List_vector<T>::operator[](int i) const
{
	return const_cast<List_vector<T>&>(*this)[i - 1];
}
